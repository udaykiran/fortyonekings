...
# RCS Identication ; $Revision: 5565 $ ; $Date: 2009-02-19 15:43:33 +0100 (jeu, 19 f√©v 2009) $ 
#
...
sub DoFile {
    ...
    #_amr ici on ne doit prendre que la premiËre ligne !
    ...
    if ($main::daemon_usage == DAEMON_CREATION || (($Conf{'automatic_list_feature'} eq 'on') && $main::daemon_usage == DAEMON_ALL )) {
		...
		if ($Conf{'automatic_list_removal'} =~ /if_empty/i) {
		    ...
		    if (exists $List::list_of_lists{$list->{'domain'}}{$list->{'name'}}) { # test ‡ virer si ok
		      delete $List::list_of_lists{$list->{'domain'}}{$list->{'name'}};
		      &do_log('err', 'la liste a ÈtÈ trouvÈe dans la list_of_lists',$listname, $dyn_list_family);
		    }
		}
                return undef;
            }
